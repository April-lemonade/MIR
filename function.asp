...
<% Function checkMirPassword(name, password)
	Dim Conn, RS, SQL	'Local variables!
	Set Conn = Server.CreateObject("ADODB.Connection")
	' memberDb is defined in constant.inc
	Conn.Open "DBQ=" & Server.MapPath(memberDb) & ";Driver={Microsoft Access Driver (*.mdb)};Driverld=25;FIL=MS Access;"
	SQL = "select * from MIR where (chineseName='" & name & "'and userpassword='" & password & "')"
rem	SQL = "SELECT * FROM MIR WHERE (  ((MIR.ChineseName)='郭智超') AND ((MIR.Birthday)='19781218'))"
